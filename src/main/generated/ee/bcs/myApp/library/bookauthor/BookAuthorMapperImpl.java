...
public class BookAuthorMapperImpl implements BookAuthorMapper {
    ...
    @Override
    public List<BookAuthorDto> toDtos(List<BookAuthor> bookAuthor) {
        if ( bookAuthor == null ) {
            return null;
        }

        List<BookAuthorDto> list = new ArrayList<BookAuthorDto>( bookAuthor.size() );
        for ( BookAuthor bookAuthor1 : bookAuthor ) {
            list.add( toDto( bookAuthor1 ) );
        }
